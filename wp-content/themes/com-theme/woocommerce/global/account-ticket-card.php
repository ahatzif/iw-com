...
<article class="group overflow-hidden rounded-[1.2rem] border border-blue/15 <?= $card_args['size'] === 'large' ? 'bg-ochre-light' : 'bg-white' ?>">
    <div class="grid md:grid-cols-[24rem_1fr] lg:grid-cols-[28rem_1fr]">
        <a href="<?= esc_url( $order_url ?: get_permalink( $ticket_post ) ) ?>" <?= $order_url ? 'data-barba-prevent data-account-pages="link"' : '' ?> class="block aspect-[4/3] overflow-hidden bg-ochre-light md:aspect-auto md:min-h-[22rem]">
            <?php if ( $image_id ) : ?>
                <?= wp_get_attachment_image( $image_id, 'large', false, [ 'class' => 'size-full object-cover transition-transform duration-500 group-hover:scale-[1.03]' ] ) ?>
            <?php else : ?>
        ...
        <div class="flex min-w-0 flex-col justify-between gap-25 p-20 md:p-25">
            <div>
                <div class="mb-15 flex flex-wrap items-center justify-between gap-10">
                    <span class="text-[1rem] font-medium uppercase tracking-[.16em] text-blue-soft"><?= esc_html( $type_label ) ?></span>
            ...
            <div class="flex flex-col gap-20 border-t border-dashed border-blue/25 pt-20 sm:flex-row sm:items-end sm:justify-between">
                <dl class="grid grid-cols-2 gap-x-25 gap-y-10 text-[1.3rem]">
                    <?php if ( $slot_start instanceof DateTimeInterface ) : ?>
                        <div>
                            <dt class="text-[.9rem] uppercase tracking-[.1em] text-blue/50"><?= esc_html__( 'Ημερομηνία', 'com-theme' ) ?></dt>
                            <dd class="m-0 mt-4 font-bold"><?= esc_html( wp_date( 'd/m/Y', $slot_start->getTimestamp() ) ) ?></dd>
                        </div>
                        <div>
                            <dt class="text-[.9rem] uppercase tracking-[.1em] text-blue/50"><?= esc_html__( 'Ώρα', 'com-theme' ) ?></dt>
                    ...
                    <?php endif; ?>
                    <div>
                        <dt class="text-[.9rem] uppercase tracking-[.1em] text-blue/50"><?= esc_html__( 'Εισιτήρια', 'com-theme' ) ?></dt>
