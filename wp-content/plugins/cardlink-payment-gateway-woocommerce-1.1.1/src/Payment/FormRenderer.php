...
class FormRenderer {
    ...
    private function render_auto_submit_js( bool $use_iframe ): string {
        ...
        $delay_ms = max(
            0,
            (int) apply_filters( 'cardlink_payment_redirect_delay', 0, $use_iframe )
        );
        $show_block_ui = (bool) apply_filters(
            'cardlink_payment_redirect_block_ui',
            ! $use_iframe,
            $use_iframe
        );
        ...
        $js .= 'var submitCardlinkPayment=function(){var form=document.getElementById("payment_form");if(form){form.submit();}};';

        if ( $delay_ms > 0 ) {
            $js .= 'window.setTimeout(submitCardlinkPayment,' . $delay_ms . ');';
        } else {
            $js .= 'submitCardlinkPayment();';
        }
