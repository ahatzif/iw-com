...
if( ! empty( $faqs ) ){  ?>
    ...
    <div class="<?php echo esc_attr( com_theme_block_wrapper_classes() ); ?>">
        <div class="mx-1/12 md:mx-2/12 " data-module-accordion>
            <div class="border-current border-t-[1px] mt-[5.7rem]">
                <?php
                foreach ($faqs as $key => $faq){ ?>
                    <div class="border-current border-b-[1px] py-40" data-accordion="parent">
                        <div class="text-1 font-bold flex justify-between cursor-pointer" data-accordion="toggle">
                            ...
                            <div class="h-[2.5rem] w-[2.5rem] ml-30 relative flex items-center flex-shrink-0">
                                <div class="w-full h-[3px] bg-current rounded-full"></div>
                                <div class="w-full h-[3px] bg-current rounded-full absolute origin-center top-1/2 left-1/2 transition duration-300 -translate-x-1/2 -translate-y-1/2 <?php if( $key !== 0 ) echo 'rotate-90' ?>" data-class-toggle="rotate-90"></div>
                            </div>
                        </div>
